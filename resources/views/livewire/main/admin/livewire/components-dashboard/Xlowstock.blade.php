<div class="p-3 rounded-xl text-white shadow-md shadow-gray-400" style="background-color: rgb(223, 87, 123);">
    <div class="flex flex-row justify-center items-center space-x-3">
        ...
        <section class="">
            <h3 class="text-base">Low Stock Products</h3>
            <h1 class="text-2xl font-bold">
                {{ !isset($products) ? '0' : count($products) }}
            </h1>
        </section>
    </div>
    @isset($products)
        <div class="mt-2 rounded-lg overflow-hidden border border-white">
            <table class="w-full m-0 text-sm border border-white border-collapse table-fixed table-dashboard">
                <thead>
                    <tr class="tracking-wide font-normal bg-white" style="color: rgb(223, 87, 123);">
        ...
        @if (count($products) > 10)
            <div class="text-xs text-center">and {{ count($products) - 10 }} more...</div>
        @endif
    @endisset
</div>
