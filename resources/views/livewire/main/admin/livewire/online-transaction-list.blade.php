    ...
    <div class="w-full h-96 overflow-y-auto mb-5" id="orders-container">
        <ul class=" w-full flex flex-col items-center">
            ...
            <li class="w-full flex justify-center select-none px-2" wire:key="{{ $transaction->id }}">
                ...
                <label class="w-11/12 py-2 my-1 rounded-full border-3 border-gray shadow-sm text-sm flex items-center text-white
                            @if($transaction->status === 'Processing') bg-orange-400
                            @elseif($transaction->status === 'On Hold') bg-yellow-500
                            @elseif($transaction->status === 'Cancelled') bg-rose-400
                            @elseif($transaction->status === 'Returned') bg-gray-500
                            @elseif($transaction->status === 'In Transit') bg-cyan-600
                            @elseif($transaction->status === 'Ready for Pickup') bg-pink-400
                            @elseif($transaction->status === 'Complete') bg-teal-500
                            @endif" for="transactionId{{ $transaction->id }}">
