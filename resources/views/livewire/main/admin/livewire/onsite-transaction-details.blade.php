<div class="h-96">
            ...
            <div class="w-full h-full flex flex-col">
                <div class="w-full flex-row ">
                    <ul class="flex flex-row w-full mt-3">
                        ...
                        <li class="w-3/12 text-center text-xs font-semibold pl-12">Unit Price</li>
                        <li class="w-2/12 text-center text-xs font-semibold pl-8">Qty</li>
                ...
                <div class="w-full flex-row my-1">
                    <ul class="flex flex-row w-full">
                        <li class="w-3/12 text-left text-xs flex items-center justify-center ">
                        ...
                        <li class="w-3/12 ml-[-2.5rem] flex text-left text-xs items-center justify-start pr-1">
                            {{ $detail->products->name }}
                        </li>
                        <li class="w-3/12 text-left text-xs flex items-center justify-center">₱ {{ number_format($detail->products->price, 2) }}</li>

                        <li class="w-1/12 text-left text-xs flex items-center justify-center">
                            {{ $detail->quantity }}
                        </li>
                        <li class="w-3/12 text-left text-xs flex items-center justify-center">₱ {{ number_format($subtotal, 2) }}</li>
    ...
    <div class="w-full mt-4 flex justify-end">
        <a href="{{ route('edittransactions', ['transactionId' => $selectedTransaction]) }}" class="h-9 px-6 flex flex-row items-center justify-center rounded-lg bg-gray-500 ml-3 border-1 border-black text-black text-sm font-semibold text-spacing">
            Update&nbsp;Transaction
