<div>
    <div class="w-full h-96 overflow-y-auto mb-5" id="transactions-container">
        <ul class=" w-full flex flex-col items-center">
            ...
            <div class="block lg:hidden">
                ...
                <livewire:onsite-transaction-details transId="{{ $transaction->id }}" wire:key="{{ $transaction->id }}" />
                @endif
            </div>

            <li wire:key="{{ $transaction->id }}" wire:click="selectTransaction({{ $transaction->id }})" class="w-full flex justify-center select-none px-2">
                {{-- Product Details --}}
    ...
    <div class="flex flex-row items-center mt-4">
        <!-- NEW PAGINATION AND NEW TRANSACTION BUTTON -->
