<div class="border-black border-1 w-full rounded-lg">
    ...
    <form wire:submit.prevent class="h-full w-full" enctype="multipart/form-data">
        <div class="columns-2">
            <div class="p-2">
                <img src="{{ $newlyUploadedImage ? $newImage->temporaryUrl() : (filter_var($selectedProduct->image, FILTER_VALIDATE_URL) ? $selectedProduct->image : asset('storage/assets/' . $selectedProduct->image)) }}" alt="{{ $selectedProduct->name }}" style="max-height: 200px;">
            </div>
        ...
        <div class="px-2 w-full">
            ...
            <div class="absolute top-0 left-0 w-full h-full flex items-center justify-center border">
                ...
                <div class="bg-gray-100 p-6 rounded-lg relative z-10 border">
                    <p class="text-xs text-gray-800 mb-4 font-medium">Are you sure you want to delete this product?</p>
                    <div class="flex justify-end">
                        <button type="button" wire:click="deleteProduct" class="px-4 py-2 bg-red-600 text-white rounded-md mr-2">Yes</button>
                        <button type="button" wire:click="$set('confirmDelete', false)" class="px-4 py-2 bg-gray-400 text-white rounded-md">No</button>
                    </div>
                </div>
            </div>
            ...
            <div class="absolute top-0 left-0 w-full h-full flex items-center justify-center">
                ...
                <div class="bg-gray-100 p-6 rounded-lg relative z-10">
                    <p class="text-xs text-gray-800 mb-4 font-medium">Are you sure you want to update this product?</p>
                    <div class="flex justify-end">
                        <button type="button" wire:click="updateProduct" class="px-4 py-2 bg-sky-600 text-white rounded-md mr-2">Yes</button>
                        <button type="button" wire:click="$set('confirmUpdate', false)" class="px-4 py-2 bg-gray-400 text-white rounded-md">No</button>
                    </div>
                </div>
            </div>
            @endif
    ...
    <button wire:click="updateConfirm" type="submit" @if($confirmDelete || $confirmUpdate) disabled @endif class="h-10 flex-1 items-center justify-center rounded-lg bg-sky-600 ml-3 border-1 border-black text-white text-sm font-semibold text-spacing flex flex-row">
        Update
