    ...
    <li class="w-full flex justify-center select-none px-2 my-3 mt-4">
        ...
        <ul class="flex flex-row w-full">
            ...
            <li class="w-3/12 text-center text-sm flex items-center relative">
                <div class="relative flex flex-col items-center max-w-[6rem]">
                    <div class="flex h-full w-full">
                        <button wire:click="decrementQuantity({{ $entry->product->id }})" type="button" id="decrement-button" data-input-counter-decrement="quantity-input" class="bg-white dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-sm p-2 h-8 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
                        ...
                        <input type="number" id="quantity-input-{{ $entry->product->id }}" data-input-counter data-stock-quantity="{{ $entry->product->stockquantity }}" aria-describedby="helper-text-explanation" class="bg-white border-gray-300 h-8 text-center text-gray-900 text-sm focus:ring-blue-500 focus:border-blue-500 block w-full py-1.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-black dark:focus:ring-blue-500 dark:focus:border-blue-500" value='{{ $entry->quantity }}' required>
                        <button wire:click="incrementQuantity({{ $entry->product->id }})" type="button" id="increment-button" data-input-counter-increment="quantity-input" class="bg-white dark:bg-gray-700 dark:hover:bg-gray-600 dark:border-gray-600 hover:bg-gray-200 border border-gray-300 rounded-sm p-2 h-8 focus:ring-gray-100 dark:focus:ring-gray-700 focus:ring-2 focus:outline-none">
            ...
            <li class="w-1/12 text-center text-sm flex items-center">
                <button wire:click="removeItem({{ $entry->product->id }})" class=" h-full w-10 flex items-center justify-center">
